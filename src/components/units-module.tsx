...
import {
  fetchUnits, createUnit, fetchProperties, fetchHostProperties, fetchAllProperties,
  ...
  type Unit, type MasterItem, supabase
} from '@/lib/supabase';
...
export function UnitsModule({ role }: UnitsModuleProps) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      let p;
      if (role === 'admin') {
         p = await fetchAllProperties();
      } else {
         p = await fetchHostProperties(hostId);
      }

      // If prop-mgr or owner, filter units by their properties
      const u = await fetchUnits(); // TODO: Add property filters to fetchUnits for performance
      const pIds = (p || []).map(prop => prop.id);
      
      const filteredUnits = role === 'admin' ? (u || []) : (u || []).filter(unit => pIds.includes(unit.property_id));
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground mt-1">Manage all rental and sale units across your portfolio.</p>
        </div>
