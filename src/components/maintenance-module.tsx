...
export function MaintenanceModule({ role }: MaintenanceModuleProps) {
  ...
  const load = useCallback(async () => {
    try {
      ...
      // In a real app, use role and userId to filter appropriately.
      // For demo, we just fetch all if admin, or use dummy filter if prop-mgr
      const data = await fetchMaintenanceTickets(role === 'admin' ? {} : { host_id: userId || MOCK_HOST_ID });
      if (data.length > 0) {
        setTickets(data);
