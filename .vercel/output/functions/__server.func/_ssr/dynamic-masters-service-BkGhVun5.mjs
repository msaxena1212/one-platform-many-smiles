//#region node_modules/.nitro/vite/services/ssr/assets/dynamic-masters-service-BkGhVun5.js
var MASTER_DEFINITIONS = [
	{
		key: "gender",
		label: "Gender",
		category: "Employee",
...
var LOCAL_STORAGE_KEY = "zyno_custom_dynamic_masters_v1";
var MANDATORY_FLAGS_KEY = "zyno_master_mandatory_flags_v1";
var DynamicMastersService = class {
	...
	static setMandatoryFlag(masterKey, isMandatory) {
		if (typeof window === "undefined") return;
	...
	static isMasterMandatory(masterKey) {
		const flags = this.getMandatoryFlags();
		if (flags[masterKey] !== void 0) return flags[masterKey];
		return MASTER_DEFINITIONS.find((d) => d.key === masterKey)?.isMandatoryField ?? false;
	}
	static getMasterValues(masterKey) {
		const customList = this.getStoredData()[masterKey];
	...
	static getMasterStringOptions(masterKey) {
		return this.getMasterValues(masterKey).map((m) => m.name);
	}
	static addMasterValue(masterKey, name) {
		const trimmed = name.trim();
		if (!trimmed) throw new Error("Name cannot be empty");
		const stored = this.getStoredData();
		const current = this.getMasterValues(masterKey);
		if (current.some((c) => c.name.toLowerCase() === trimmed.toLowerCase())) throw new Error(`Item "${trimmed}" already exists in ${masterKey}.`);
		const newEntry = {
	...
	static updateMasterValue(masterKey, id, newName) {
		const trimmed = newName.trim();
		if (!trimmed) throw new Error("Name cannot be empty");
		const stored = this.getStoredData();
		stored[masterKey] = this.getMasterValues(masterKey).map((item) => item.id === id ? {
			...item,
			name: trimmed
		} : item);
		this.setStoredData(stored);
	}
	static deleteMasterValue(masterKey, id) {
		const stored = this.getStoredData();
		stored[masterKey] = this.getMasterValues(masterKey).filter((item) => item.id !== id);
		this.setStoredData(stored);
	}
};
//#endregion
export { MASTER_DEFINITIONS as n, DynamicMastersService as t };
